...
class CompanyControllerTest {
  ...
  static PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>(
      "postgres:17").withDatabaseName("testdb").withUsername("user").withPassword("password");
  ...
  void shouldReturn_200_getCompanyById() throws Exception {
    mockMvc.perform(get("/" + Path.API_COMPANIES + "/" + companyId).with(withJwt()))
  ...
  void shouldReturn_400_getCompanyById() throws Exception {
    mockMvc.perform(get("/" + Path.API_COMPANIES + "/invalid-uuid").with(withJwt()))
  ...
  void shouldReturn_401_getCompanyById() throws Exception {
    mockMvc.perform(get("/" + Path.API_COMPANIES + "/" + companyId))
  ...
  void shouldReturn_403_getCompanyById() throws Exception {
    mockMvc.perform(get("/" + Path.API_COMPANIES + "/" + companyId).with(withBadJwt()))
  ...
  void shouldReturn_404_getCompanyById() throws Exception {
    mockMvc.perform(
        ...
        .andExpect(content().contentType(MediaType.APPLICATION_JSON)).andExpect(
            jsonPath("$.message").value(
                "Company not found with id: cf578fec-006b-4604-a5e8-5ad1b2ea2be5"));
  }

  @Test
  void shouldReturn_201_createCompany() throws Exception {
    CompanyCreateRequestDTO request = CompanyCreateRequestDTO.builder().name("Company test")
        .nip("1234567890").regon("123456789").website("https://company-test.com").build();
    String requestBody = objectMapper.writeValueAsString(request);
    mockMvc.perform(
            post("/" + Path.API_COMPANIES).with(withJwt()).contentType(MediaType.APPLICATION_JSON)
  ...
  void shouldReturn_400_createCompany() throws Exception {
    CompanyCreateRequestDTO request = CompanyCreateRequestDTO.builder().name("Co").nip("test")
        .regon("test").website("http://company-test.com").build();
    String requestBody = objectMapper.writeValueAsString(request);
    mockMvc.perform(
            post("/" + Path.API_COMPANIES).with(withJwt()).contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(jsonPath("$.errors.name").value("Name must be 3-255 characters")).andExpect(
            jsonPath("$.errors.root.serverError").value(
  ...
  void shouldReturn_401_createCompany() throws Exception {
    ...
    String requestBody = objectMapper.writeValueAsString(request);
    mockMvc.perform(
            post("/" + Path.API_COMPANIES).contentType(MediaType.APPLICATION_JSON).content(requestBody))
        .andExpect(status().isUnauthorized())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON)).andExpect(
            jsonPath("$.message").value("Full authentication is required to access this resource"));
  }
  ...
  void shouldReturn_403_createCompany() throws Exception {
    ...
    String requestBody = objectMapper.writeValueAsString(request);
    mockMvc.perform(
            post("/" + Path.API_COMPANIES).with(withBadJwt()).contentType(MediaType.APPLICATION_JSON)
  ...
  void shouldReturn_200_getCompanyList() throws Exception {
    ...
    List<CompanySimpleDTO> responseBody = objectMapper.readValue(contentAsString,
        new TypeReference<>() {
        });
